...
| 平台 | 入门月费 | 核心模型 | 5小时额度 | 形象对比 | 估算单条成本（vs Claude） | 推荐 |
|---|---|---|---|---|---|---|
| **Claude 官方** | ¥145（$20） | Opus 4.6, Sonnet 4.6 | ~45 prompts | 每天一杯美式 | ~¥0.07/条（基准 1×） | 复杂推理、论文写作、架构设计 |
| **MiniMax** | ¥29（$4） | M2.5, M2.5-highspeed | 100 prompts | 每月一杯瑞幸 | ~¥0.007/条（Claude 的 **1/10**） | 大量日常编码、数据清洗脚本 |
| **智谱 GLM** | ¥49（$7） | GLM-5, GLM-4.7 | ~80 prompts | 每月两杯奶茶 | ~¥0.014/条（Claude 的 **1/5**） | 需要联网搜索和图片理解的任务 |
| **Kimi** | ~¥200（$28） | Kimi K2.5 | 300 prompts | 每月一顿火锅 | ~¥0.015/条（Claude 的 **1/5**） | 长上下文、高速推理（100 TPS） |
| **通义千问** | ¥72（$10） | Qwen3.5-Plus 等 6 模型 | ~100 prompts | 每月一顿外卖 | ~¥0.016/条（Claude 的 **1/4**） | 需要在多个模型间切换比较 |

> **成本估算方法**：单条成本 = 月费 ÷（5小时额度 × 每月活跃窗口数）。按每天 2 个活跃 5 小时窗口、每月 22 个工作日计算（44 窗口/月）。实际成本因使用强度而异。
>
> **一个直觉**：Claude 官方 Pro 一个月 ¥145，大约能发 2,000 条 prompt。同样的钱如果用 MiniMax，可以发约 **20,000 条**——十倍的量。代价是模型推理深度不及 Claude Opus。
...
智谱的优势在于 GLM-5 模型在 SWE-bench Verified 上表现顶尖，且附带免费 MCP 工具：
- 视觉理解（图片/UI/架构图识别）
