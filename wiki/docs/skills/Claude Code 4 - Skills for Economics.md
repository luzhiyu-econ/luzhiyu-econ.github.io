...
> Skills 是 Claude Code 的"技能树"——把你反复使用的工作流打包成可复用的模块，一次设计，永久受益。

你是否经历过这样的场景：每次做回归诊断都要重复告诉 Claude 检查平行趋势、F 统计量、标准误聚类……为什么不把这些打包成一个"技能"，以后一句 `/regression-diagnostics DID` 就全部搞定？这就是 Skills 的价值。
