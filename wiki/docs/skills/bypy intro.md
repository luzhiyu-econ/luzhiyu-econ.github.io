...
这意味着你需要先将要下载的文件**移动或复制**到网盘的 `/apps（我的应用数据）/bypy/` 目录下。可以通过百度网盘网页版或手机 App 操作。

> **提示：** 运行 `bypy list` 后，如果 `/apps（我的应用数据）/bypy/` 目录不存在，bypy 会自动创建一个空目录。或者也可以自己提前创建一个对应文件夹，并将数据复制进去。
